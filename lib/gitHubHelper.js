...
function createCheckRunWithScanResult(trivyStatus) {
...
function getCheckSummary(trivyStatus) {
    let summary = `Container scan report for \`${inputHelper.imageName}\`:`;
    const trivySummary = trivyHelper.getOutputSummary(trivyStatus);
    summary = summary + '\n' + trivySummary;
    return summary;
}
function getCheckText(trivyStatus) {
    let text = '';
