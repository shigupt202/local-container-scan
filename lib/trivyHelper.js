...
function getVulnerabilityText(trivyStatus) {
    const vulnerabilityIds = getVulnerabilityIds(trivyStatus);
    return vulnerabilityIds.join('\n');
}
exports.getVulnerabilityText = getVulnerabilityText;
function getOutputSummary(trivyStatus) {
    let summary = '';
    switch (trivyStatus) {
        case 0:
