...
const stableTrivyVersion = "v.0.5.0";
...
function getTrivyDownloadUrl(trivyVersion: string): string {
    ...
    switch (curOS) {
        case "Linux":
            return util.format("https://github.com/aquasecurity/trivy/releases/download/%s/trivy_%s_Linux-32bit.tar.gz", trivyVersion, trivyVersion);

        case "Darwin":
            return util.format("https://github.com/aquasecurity/trivy/releases/download/%s/trivy_%s_macOS-32bit.tar.gz", trivyVersion, trivyVersion);
